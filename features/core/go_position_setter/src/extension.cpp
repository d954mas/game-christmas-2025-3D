...
class PositionSetterUserdata {
  public:
    ...
    void addInstance(dmGameObject::HInstance rootInstance, dmVMath::Vector3 *position);
    void update();
...
void PositionSetterUserdata::addInstance(dmGameObject::HInstance rootInstance, dmVMath::Vector3 *position) {
...
static int LuaPositionSetterUserdataAddInstance(lua_State *L) {
    ...
    userdata->addInstance(rootInstance, position);
    return 0;
}
