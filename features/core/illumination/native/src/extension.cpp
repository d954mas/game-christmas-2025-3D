...
static int FrustumIsBoxVisibleLua(lua_State* L){
    ...
    dmVMath::Vector3 minv(luaL_checknumber(L, 2),luaL_checknumber(L, 3),luaL_checknumber(L, 4));
    dmVMath::Vector3 maxv(luaL_checknumber(L, 5),luaL_checknumber(L, 6),luaL_checknumber(L, 7));

    Frustum frustum = Frustum(*m);

    const bool visible = frustum.IsBoxVisible(minv, maxv);
