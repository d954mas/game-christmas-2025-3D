...
//in highp vec4 var_clip_position;
  #include "/features/core/illumination/assets/materials/shadow/shadow_fp_in.glsl"

// Блок обычных uniform'ов
uniform fs_uniforms {
    lowp vec4 tint;
    lowp vec4 phong;                // x: shininess, y: specular strength
...
void main() {
    ...
    #ifdef EDITOR
    out_fragColor = vec4(texture_color.rgb, texture_color.a);
    return;
    #endif
   ...
   //do not calculate lights
    ...
    // Тень
    float shadow = shadow_calculation_with_added_normal_bias(var_texcoord0_shadow, var_world_normal, var_world_position);
    vec3 shadowMod = shadow_color.rgb * shadow_color.a * shadow;

    // Освещение от солнца + результат
    float shininess = phong.x;
    float specularStrength = phong.y;

    vec3 resultColor = color * ambient;
    resultColor += color * max(
        direct_light_phong(sunlight_color, sun_position.xyz,
                           var_world_position, surface_normal,
                           shadowMod, var_camera_position,
                           shininess, specularStrength), 0.0
    );
    ...
    // Screen-space dithering shifts fog slightly per-pixel to hide 8-bit banding
