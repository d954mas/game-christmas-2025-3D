...
local HASH_SHADOW = hash("shadow")
local HASH_SHADOW_BONES = hash("shadow_bones")
...
local function create_empty_shadow_buffer()
	...
	return render.render_target("empty_shadow_buffer", { [graphics.BUFFER_TYPE_COLOR0_BIT] = color_params })
end

---@class Lights
local Lights = CLASS.class("lights")

function Lights.new() return CLASS.new_instance(Lights) end

function Lights:initialize()
	illumination.lights_init(1, 1, 1, 1, 2)
	...
	self.shadow = {
		...
		rt = nil,
		rt_no_shadow = nil,
		no_shadow_clear_color = { [graphics.BUFFER_TYPE_COLOR0_BIT] = vmath.vector4(1),
		...
		draw_transient = { transient = { graphics.BUFFER_TYPE_DEPTH_BIT } },
		draw_clear = { [graphics.BUFFER_TYPE_COLOR0_BIT] = vmath.vector4(1, 1, 1, 1), [graphics.BUFFER_TYPE_DEPTH_BIT] = 1 }
	}
...
function Lights:init(data_texture_resource)
...
function Lights:draw_begin()
	---@diagnostic disable-next-line: param-type-mismatch
	render.enable_texture("SHADOW_TEXTURE", self.shadow.rt or self.shadow.rt_no_shadow, graphics.BUFFER_TYPE_COLOR0_BIT)
end

function Lights:draw_finish()
	render.disable_texture("SHADOW_TEXTURE")
end
...
function Lights:set_render(render_obj)
	...
	self.shadow.pred_front_culling = render.predicate({ "shadow_front_culling" })
	self.shadow.pred_flipped = render.predicate({ "shadow_flipped" })
	self.shadow.pred_bones = render.predicate({ "shadow_bones" })
...
function Lights:reset()
	...
	self:set_shadow_color(0.05, 0.05, 0.05)
	self:set_shadow_color_intensity(1)

	self:set_ambient_color(1, 1, 1)
	self:set_ambient_color_intensity(0.8)

	self:set_fog(150, 250, 0.9)
	self:set_fog_color(135 / 255, 206 / 255, 255 / 255)

	self:set_sun_position(-10, 30, 8)
end
...
function Lights:set_camera(x, y, z)
	if self.need_reset > 0 then
		self.need_reset = self.need_reset - 1
		self.shadow.light_projection_bounds.x = 0
		self.shadow.light_projection_bounds.y = 0
		self.shadow.light_projection_bounds.z = 0
		self.shadow.light_projection_bounds.w = 0
	end
	...
	for _, point in ipairs(POINTS_CUBE_RESULT) do
		--convert new coordinates to current shadow projection
		xmath.matrix_mul_v4(TEMP_V4, self.shadow.light_matrix_projection_bounds, point)
	...
	--	print("shadow uv new check:x[" .. min_x .. " " .. max_x .. "] y[" .. min_y .. " " .. max_y .. "] w:" .. max_x - min_x .. " h:" .. max_y - min_y)
	...
	if min_x < self.shadow.light_projection_bounds.x or max_x > self.shadow.light_projection_bounds.y
		...
		min_x, max_x = math.huge, -math.huge
		min_y, max_y = math.huge, -math.huge
		for _, point in ipairs(POINTS_CUBE_RESULT) do
			--convert new coordinates to new shadow projection
		...
		xmath.matrix_from_matrix(self.shadow.light_matrix_projection_bounds, MATRIX_LIGHTS)
...
function Lights:set_frustum(frustum)
	...
	illumination.lights_set_frustum(self.frustum)
end

function Lights:set_view(view)
	self.view = view
	illumination.lights_set_view(self.view)
end
