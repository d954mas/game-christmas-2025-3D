...
uniform mediump sampler2D EMISSION;
...
void main() {
	...
	vec4 emission_sample = texture(EMISSION, var_texcoord0);
	vec3 emission_rgb = emission_sample.rgb * tint.rgb;
	float emission_mask = emission_sample.a;
	const float EMISSION_INTENSITY = 0.35;

#ifdef EDITOR
	vec3 editor_color = max(texture_color.rgb, emission_rgb);
	editor_color += emission_rgb * emission_mask * EMISSION_INTENSITY;
	out_fragColor = vec4(editor_color, texture_color.a);
	return;
#endif
	...
	resultColor = max(resultColor, emission_rgb);
	resultColor += emission_rgb * emission_mask * EMISSION_INTENSITY;
