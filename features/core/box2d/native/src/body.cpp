...
Body* Body_get_userdata_unsafe(lua_State *L, int index){
    Body *lua_body = (Body*) BaseUserData_get_userdata_unsafe(L, index);
    return lua_body;
}
...
static int GetPositionRawScaled(lua_State *L){
    utils::check_arg_count(L, 2);
    float scale =  lua_tonumber(L,2);
    Body *body = Body_get_userdata_safe(L, 1);
    const b2Vec2& position = body->body->GetPosition();
    lua_pushnumber(L, position.x/scale);
    lua_pushnumber(L, position.y/scale);
    return 2;
}
...
static int GetLinearVelocityRaw(lua_State *L){ //const b2Vec2& GetLinearVelocity () const
    utils::check_arg_count(L, 1);
    Body *body = Body_get_userdata_safe(L, 1);
    b2Vec2 vector = body->body->GetLinearVelocity();
...
void BodyInitMetaTable(lua_State *L){
    ...
    luaL_Reg functions[] = {
        ...
        {"GetPositionRawScaled",GetPositionRawScaled},
        {"GetAngle",GetAngle},
        ...
        {"GetLinearVelocityRaw",GetLinearVelocityRaw},
