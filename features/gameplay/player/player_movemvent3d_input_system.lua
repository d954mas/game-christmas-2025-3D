...
---@class PlayerMovement3dInputSystem:EcsSystem
local System = CLASS.class("PlayerMovement3dInputSystem", ECS.System)
...
function System:draw(_)
    if INPUT.IGNORE or SM:get_top()._name ~= "GameScene" or SDK.show_ad or SM:is_working() then
        ...
        self.move_forward = (INPUT.get_key_data(HASHES_INPUT.ARROW_UP).pressed or INPUT.get_key_data(HASHES_INPUT.W).pressed) and 1 or 0
        self.move_backward = (INPUT.get_key_data(HASHES_INPUT.ARROW_DOWN).pressed or INPUT.get_key_data(HASHES_INPUT.S).pressed) and 1 or 0
        ...
        self.move_up = INPUT.get_key_data(HASHES_INPUT.SPACE).pressed and 1 or 0
        self.move_down = INPUT.get_key_data(HASHES_INPUT.LEFT_SHIFT).pressed and 1 or 0
    end
    ---@class Entity
    ...
    movement.input.z = self.move_backward - self.move_forward

    movement.max_speed_limit = 1

    if INPUT.get_key_data(HASHES_INPUT.SPACE).just_pressed then
        movement.pressed_jump = true
    end

    local x, y, speed_limit = VirtualPadFeature:get_data()
    if x then
        movement.input.x, movement.input.z = x, -y
        movement.input.y = 0
        movement.max_speed_limit = speed_limit
    end
    player.pressed_action = INPUT.get_key_data(HASHES_INPUT.E).pressed
end
