local MAP_HELPER = require "features.gameplay.tiled.levels.parser.map_helper"
local CLASS = require "libs.class"
local LUME = require "libs.lume"

local MATH_CEIL = math.ceil

---@class Level
local Level = CLASS.class("Level")

function Level.new(data, tilesets)
	return CLASS.new_instance(Level, data, tilesets)
end

---@param objects LevelMapObject[]
local function objects_set_meta(objects, tilesets)
	for _, object in pairs(objects) do
		if (object.tile_id and object.properties) then
			local tile = assert(tilesets.by_id[object.tile_id], "no tile with id:" .. object.tile_id)
			setmetatable(object.properties, { __index = tile.properties })
		end
	end
end

---@param data LevelData
---@param tilesets LevelTilesets
function Level:initialize(data, tilesets)
	self.name = nil
	self.data = assert(data)
	self.tilesets = assert(tilesets)
	self.cell_max_id = self.data.size.w * self.data.size.h
	objects_set_meta(self.data.game_objects, tilesets)
	objects_set_meta(self.data.visual_object, tilesets)
end

--region MAP
function Level:map_get_width() return self.data.size.w end
function Level:map_get_height() return self.data.size.h end

function Level:map_cell_id_is_valid(id)
	return id >= 1 and id <= self.cell_max_id
end

function Level:coords_is_valid(x, y)
	return x >= 0 and y >= 0 and x < self.data.size.w and y < self.data.size.h
end

function Level:coords_to_id(x, y)
	x = LUME.clamp(MATH_CEIL(x),1,self.data.size.w-1)
	y = LUME.clamp(MATH_CEIL(y),1,self.data.size.h-1)
	return MAP_HELPER.coords_to_id(self.data, x, y)
end
function Level:coords_to_id_unsafe(x, y)
	return MAP_HELPER.coords_to_id(self.data, x,y)
end

function Level:id_to_coords(id)
	local x, y = MAP_HELPER.id_to_coords(self.data, id)
	return x, y
end

--endregion


---@return LevelMapTile
function Level:get_tile(id)
	return assert(self.tilesets.by_id[id], "no tile with id:" .. id)
end

---@return LevelMapTile
---@return number
function Level:get_tile_for_tileset(tileset_name, id)
	assert(tileset_name)
	assert(id)
	local tileset = assert(self.tilesets.tilesets[tileset_name], "no tileset with name:" .. tileset_name)
	local tile_id = tileset.first_gid + id
	assert(tile_id <= tileset.end_gid, "no tile:" .. tile_id .. " in tileset:" .. tileset_name .. " end:" .. tileset.end_gid)
	return self:get_tile(tile_id), tile_id
end

return Level