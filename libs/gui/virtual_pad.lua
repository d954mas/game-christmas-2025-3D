...
function VirtualPad:update(dt)
    --[[
   if (INPUT.TOUCH) then
        DEBUG_LOG_STRING = DEBUG_LOG_STRING .. "touch: " .. tostring(INPUT.TOUCH.id) .. "\n"
    else
        DEBUG_LOG_STRING = DEBUG_LOG_STRING .. "touch: nil\n"
    end
    for i = 1, #INPUT.TOUCH_MULTI do
        DEBUG_LOG_STRING = DEBUG_LOG_STRING .. "touch_multi: " .. tostring(INPUT.TOUCH_MULTI[i].id) .. "\n"
    end--]]
    if (self:visible_is()) then
    ...
    -- DEBUG_LOG_STRING = DEBUG_LOG_STRING .. "pad touch_id: " .. tostring(self.touch_id) .. "\n"
end
