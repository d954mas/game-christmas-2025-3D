...
local Action = CLASS.class("ParallelAction", BaseAction)
...
function Action:act(dt)
	for i = #self.sequence, 1, -1 do
		local action = self.sequence[i]
		if action:update(dt) then table.remove(self.sequence, i) end
	end

	return self.drop_empty and #self.sequence == 0
end
