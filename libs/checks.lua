...
local function is_subclass(t, name)
	...
	return CLASS.is_instance_of_by_name(t, name)
end
...
local function check_string_type(value, expected_type)
	...
	-- argname and function name are formatted by the caller
			'%s', '%s', expected_type, type(value) ~="userdata" and value and value.__class and ("class:" .. value.__class.name) or type(value)
	)
end
