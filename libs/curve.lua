...
function Curve.new(config) return CLASS.new_instance(Curve, config) end
...
function Curve:initialize(config)
    ...
    self.segments_count = config.segments or 16
...
function Curve:_calculate()
    ...
    local st              --/ steps based on num. of segments
    ...
    for i = 2, pl, 1 do
        ...
        for t = 0, self.segments_count-1 do

            --/ calc step
            st = t / self.segments_count
